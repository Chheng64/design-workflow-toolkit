...
- **`ECOSYSTEM.md`.** What "works with your existing workflow" means, stated structurally rather than as a partner list: every artifact is markdown, HTML, JSON or CSV on disk, every check is a zero-dependency Node script with an exit code, so anything that reads a file or runs a command can participate. Six categories — AI models, documentation, design, development, communication, automation — each claim backed by an existing document, with `checks.yml` as the in-repo proof of the automation one. A **Future connectors** section holds the planned items — the `dtk` CLI, the shared artifact store, the Phase 5 packs — each linking its roadmap phase, so the line between "works today" and "planned" stays visible. Named `ECOSYSTEM.md` rather than `INTEGRATIONS.md` because the page is about compatibility, not built-in connectors; the toolkit ships none, and that is the point, not a gap.
...
- **`README.md` gained a Process flow section**, between "What you'll build" and "Build your first project". One Mermaid overview of the whole run — brief, workflow, artifacts, validators and human gates, handoff, package — then the four claims that carry it in existing repo language, a one-row comparison against prompt-based AI, and the twelve states in machine order with the three human gates marked. It summarizes; `WORKFLOW_GUIDE.md` and `ARCHITECTURE.md` still hold the detail, and the section links to both. `STATE 10 · Revision` is named as the loop rather than a step, and `STATE 12` is shown before `11`, because that is the machine order.
...
- ~~**The rule candidate.**~~ **Recorded as `RC-1`** in `docs/rule-candidates.md`, deliberately **not** hardened. One occurrence is an anecdote; the register is what makes a second one recognisable as a second one. It is promoted only if the same class recurs independently, and it is cited, checked and enforced by nothing in the meantime.
